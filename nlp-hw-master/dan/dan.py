...
class DanModel(nn.Module):
    ...
    def average(self,text_embeddings, text_len):
        ...
        average = text_embeddings.sum(1).float()
        average /= text_len.view(text_embeddings.size(0), -1)
    ...
    def forward(self, input_text, text_len, is_prob=False):
        ...
        text_embedded = self.embeddings(input_text)
        # Then average them 
        avg = self.average(text_embedded, text_len)
        # Before feeding them through the network
        logits = self.model.forward(avg)
